...
上传前请确认不要提交真实密钥和本地运行产物。

建议 `.env.example` 只保留占位值：
...
```text
洁境智顾：面向智能清洁设备的环境感知 RAG Agent 系统
```

项目描述：

```text
设计并实现面向智能清洁设备场景的环境感知 RAG Agent 系统，基于 Streamlit + FastAPI 构建前后端分离架构，支持多轮会话、后台并行生成、历史记录持久化、知识库检索问答、环境清扫策略生成、参考来源追溯和 MCP 工具服务。
```

技术亮点：

- 设计企业级异步会话生成机制，基于 `ChatJob` 和后台线程池实现多会话并行生成，切换会话不影响原任务执行。
- 基于 Chroma 构建知识库检索模块，结合向量召回、关键词召回与轻量重排提升问答命中率。
- 设计 Agent 工具调用能力，接入高德定位与天气服务，并将天气、湿度、风力映射为扫地、拖地、开窗和维护策略。
- 实现会话、消息和任务状态持久化，支持刷新恢复、历史会话管理和参考来源追溯。
- 提供 MCP 工具服务入口，将 RAG、天气、环境策略和用户记录能力暴露给外部 Agent 调用。
- 使用 Redis 限流、内存降级、结构化日志和自动化测试提高系统稳定性与可维护性。
